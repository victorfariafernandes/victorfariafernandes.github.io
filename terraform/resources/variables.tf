...
variable "base_model" {
  type        = string
  description = "Ollama base model tag to pull (e.g. qwen3:0.6b)"
}

variable "model_alias" {
  type        = string
  description = "Ollama model name for the baked custom model"
}

variable "system_prompt" {
  type        = string
  description = "Security rules and behavioral constraints for the Modelfile SYSTEM directive"
}

variable "context_prompt" {
  type        = string
  description = "Domain/background context pre-seeded as conversation history"
}
